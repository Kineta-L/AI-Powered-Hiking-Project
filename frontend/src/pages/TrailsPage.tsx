...
export default function TrailsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/trails?${p}`).then(r => r.json()).then(d => { setTrails(d.trails || []); setTotal(d.total || 0); }).finally(() => setLoading(false));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="mb-8 rounded-2xl overflow-hidden border border-white/5 h-64">
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        ...
        <select value={region} onChange={e => update('region', e.target.value)}
          className="px-3 py-2 rounded-xl glass border-white/10 text-sm text-gray-300 focus:outline-none focus:border-amber-500/50">
          <option value="">{t('trails.allRegions')}</option>
          {REGIONS.map(r => <option key={r} value={r}>{r}</option>)}
        </select>
        <select value={difficulty} onChange={e => update('difficulty', e.target.value)}
          className="px-3 py-2 rounded-xl glass border-white/10 text-sm text-gray-300 focus:outline-none focus:border-amber-500/50">
          <option value="">{t('trails.allDifficulties')}</option>
          {DIFFICULTIES.map(d => <option key={d} value={d}>{t(`difficulty.${d}`, d)}</option>)}
        </select>
        <select value={season} onChange={e => update('season', e.target.value)}
          className="px-3 py-2 rounded-xl glass border-white/10 text-sm text-gray-300 focus:outline-none focus:border-amber-500/50">
          <option value="">{t('trails.allSeasons')}</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {trails.map(trail => (
              <Link key={trail.id} to={`/trails/${trail.id}`}
                ...
                <div className="h-44 bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-5xl relative">
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    <span className={`px-2 py-0.5 rounded-md text-xs border border-white/5 ${diffBadge(trail.difficulty)}`}>
                    ...
                    <span className="px-2 py-0.5 rounded-md text-xs bg-white/5 text-gray-500">{trail.region || trail.country}</span>
                  </div>
